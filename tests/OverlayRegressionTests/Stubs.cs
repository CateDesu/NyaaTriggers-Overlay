...
namespace Dalamud.Bindings.ImGui
{
    ...
    public readonly record struct DrawCommand(string Kind, string? Text, Vector2 A, Vector2 B, uint Color = 0xffffffff, float FontSize = 16);
    ...
    public static class ImGui
    {
        ...
        public static bool InvisibleButton(string label, Vector2 size) => Button(label);
        public static bool Selectable(string label) => Button(label);
        ...
        public static uint ColorConvertFloat4ToU32(Vector4 color)
            => (uint)(Math.Clamp(color.X, 0, 1) * 255) | ((uint)(Math.Clamp(color.Y, 0, 1) * 255) << 8)
                | ((uint)(Math.Clamp(color.Z, 0, 1) * 255) << 16) | ((uint)(Math.Clamp(color.W, 0, 1) * 255) << 24);
        ...
        public static bool CollapsingHeader(string label, ImGuiTreeNodeFlags flags = default)
        {
            Controls.Add(ControlId(label));
            return true;
        }
        public static void Spacing() { }
        public static void SameLine(float offset = 0) { }
        public static void SetNextItemWidth(float width) { }
        private static readonly Stack<string> IdStack = new();
        public static readonly List<string> Controls = new();
        private static string ControlId(string label) => string.Join("/", IdStack.Reverse().Append(label));
        public static void PushID(string id) => IdStack.Push(id);
        public static void PushID(int id) => PushID(id.ToString());
        public static void PopID() => IdStack.Pop();
        ...
        {
            Controls.Add(ControlId(label));
            if (Disabled != 0 || (ClickLabel != label && ClickLabel != ControlId(label))) return false;
            ClickLabel = null;
            value = !value;
            return true;
        }
        ...
        public static (string Label, int Value)? ComboInput;
        public static bool Combo(string label, ref int value, string[] names, int count)
        {
            Controls.Add(ControlId(label));
            if (ComboInput is not { } input || (input.Label != label && input.Label != ControlId(label))) return false;
            value = input.Value;
            ComboInput = null;
            return true;
        }
    ...
    public readonly struct ImDrawListPtr
    {
        public void AddCircle(Vector2 center, float radius, uint color)
            => ImGui.Commands.Add(new("circle", null, center - new Vector2(radius), center + new Vector2(radius), color));
        public void AddCircleFilled(Vector2 center, float radius, uint color)
            => ImGui.Commands.Add(new("disc", null, center - new Vector2(radius), center + new Vector2(radius), color));
        ...
        public void AddText(Vector2 position, uint color, string text)
        {
            ...
            ImGui.Commands.Add(new("text", text, position, position + ImGui.CalcTextSize(text), color, ImGui.GetFontSize()));
        }
        public void AddRectFilled(Vector2 min, Vector2 max, uint color, float rounding = 0)
            => ImGui.Commands.Add(new("rect", null, min, max, color));
        ...
        public void AddLine(Vector2 a, Vector2 b, uint color)
            => ImGui.Commands.Add(new("line", null, a, b, color));
...
namespace NyaaTriggers.Plugin.Ui
{
    ...
    internal static class JobIcons
    {
        ...
        internal static TestIcon? Get(string job, bool lmeter = false) => Available ? new TestIcon() : null;
    }
}
